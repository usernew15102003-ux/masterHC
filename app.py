...
# Added io and csv for file creation/formatting
import io
import csv
# Added Response, redirect, and url_for for file download and navigation
from flask import Flask, render_template_string, request, flash, Response, redirect, url_for

# --- Configuration ---
app = Flask(__name__)
# A secret key is required for Flask to use flash messages
app.config['SECRET_KEY'] = 'simple_secret_key_for_flash' 

# Your MySQL Connection Details
DB_USER = 'santhosh'
DB_PASSWORD = 'Santhosh@22' 
DB_HOST = '127.0.0.1' 
DB_PORT = 3306 
DB_NAME = 'training' 

# Configuration dictionary for mysql.connector
MYSQL_CONFIG = {
    'user': DB_USER,
...
# The template includes the 'Edit' button and the 'Edit Modal'.
INDEX_HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script>
    // JavaScript to populate the modal with data from the clicked row
...
def populate_site_data(conn):
    ...
    # Check if data already exists to prevent duplicate entries on every startup
    ...
    data = [
        ('APAC', 'Hub A', 'India', 'Site 1', 50, 60, 40), # Total 150
        ('APAC', 'Hub A', 'India', 'Site 2', 100, 50, 50), # Total 200
        ('EMEA', 'Hub B', 'Germany', 'Site 3', 30, 40, 30), # Total 100
        ('EMEA', 'Hub B', 'UK', 'Site 4', 150, 100, 50), # Total 300
        ('AMER', 'Hub C', 'USA', 'Site 5', 200, 150, 100), # Total 450
        ('AMER', 'Hub C', 'Canada', 'Site 6', 10, 20, 20), # Total 50
        ('APAC', 'Hub D', 'Japan', 'Site 7', 60, 30, 30), # Total 120
        ('AMER', 'Hub C', 'USA', 'Site 8', 50, 50, 50), # Total 150
    ]
...
def update_site_data(row_id, rse_count, dse_count, itc_count):
    """Updates the count columns for a specific row ID."""
...
# Function to get the row details (used for flash message after edit)
def get_site_details(row_id):
    conn = get_db_connection()
    if conn is None:
        return None
    cursor = conn.cursor(dictionary=True)
    # Only fetching location details, not counts, as counts are what we are updating/checking
...
def init_db():
    """Initializes the database by ensuring the 'site_data' table exists and is populated."""
    conn = get_db_connection()
    if conn is not None:
        cursor = conn.cursor()
        
        try:
            # Drop the table first to ensure the new schema with three count columns is applied
            cursor.execute("DROP TABLE IF EXISTS site_data")

            # Create the site_data table with ID as Primary Key
            cursor.execute('''
            ...
            conn.commit()
            print("INFO: site_data table created with RSE/DSE/ITC schema and ID column.")
            
            # Populate the site data with sample records
        ...
        print("ERROR: Failed to connect to MySQL during initialization.", file=sys.stderr)
...
def index():
    ...
    if conn is None:
        ...
        return render_template_string(INDEX_HTML_TEMPLATE, regions=[], hubs=[], countries=[], sites=[], filter_data=None)

    # Note: Using dictionary=True here so we can access column names easily
    cursor = conn.cursor(dictionary=True) 
    
    # 1. Fetch unique values for dropdowns
    try:
    ...
    # We load data on POST (form submission) or if a redirect occurred (after edit)
    if request.method == 'POST' or 'filter_data' not in request.form: 
        ...
        # 2. Build Dynamic SQL Query for Detailed Rows
        conditions = []
        params = []
        ...
        # SQL query now selects the ID in addition to the three count categories
        ...
        try:
            ...
            # Calculate the overall total count by summing RSE, DSE, and ITC from all filtered rows
            total_associates = sum(
                row['rse_count'] + row['dse_count'] + row['itc_count'] 
                for row in filtered_rows
            )

            # Prepare the result dictionary for display
            filter_data = {
...
def edit_data(row_id):
    # Fetch the data before the update for the flash message
    ...
    try:
        # Get new count values from the form
    ...
    # After update, redirect back to the main page (index)
    return redirect(url_for('index'))
...
def download_data():
    ...
    # 1. Get filter parameters from the hidden form fields
    selected_region = request.form.get('region_filter', 'All')
    selected_hub = request.form.get('hub_filter', 'All')
    selected_country = request.form.get('country_filter', 'All')
    selected_site = request.form.get('site_filter', 'All')
    ...
    # SQL query to select all matching rows including the three counts
    ...
    # 4. Format data as CSV using in-memory file handling (io.StringIO)
    output = io.StringIO()
    writer = csv.writer(output)

    # Write headers including the new categories and the total
    ...
    # 5. Return CSV as file attachment using Flask Response
...
if __name__ == '__main__':
    # Initialize the database (create table and populate data) when the app starts
    init_db()
